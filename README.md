...
- **有边界的权限**：一个开关定松紧——只读 / 工作区（默认）/ 全放行。默认工作目录里改文件不用问，
  跑命令要你按键确认；越出工作目录和明显危险的命令直接拒绝。
- **无人值守**：`sa run "..."` 跑完就退出，配合 cron / launchd 用；没人确认时，要确认的操作一律拒绝，除非 `--allow` 明确放行。
...
- `main` 每次更新，版本号最后一位自动 +1（0.2.0 → 0.2.1），`sa --version` 能看出装的是哪一版
...
- **执行者**：新建空间时选 `simpleagent`（内置 loop）、`claude-code` 或 `opencode`。
  选外部 CLI 时要先在本机装好并登录。
- **权限**：每个空间在「空间设置」里选权限模式。内置执行者三档（只读 / 工作区 / 全放行，没设过就跟配置默认），
  改了之后正在跑的会话从下一次工具调用起生效；外部 CLI 只有「只读」（默认）和「全放行」两档——
...
sa run "..." --cwd ~/projects/foo --mode 只读 --allow write_file,edit_file
```

跑完就退出，适合放进 cron / launchd。权限模式跟配置走（默认工作区：工作目录里改文件不用问），
但**不继承「全放行」**，要全放行得显式写 `--mode full`。没有人在终端前面，所以模式里要确认的操作一律按拒绝处理，
要放行就用 `--allow` 逐个列出工具名。
...
一个权限模式定下每类操作是直接放行、问你还是拒绝：

| 操作 | 只读 | 工作区（默认） | 全放行 |
|---|---|---|---|
| 读文件、搜索、只读的 MCP 工具 | 放行 | 放行 | 放行 |
| 改工作目录里的文件 | 问 | 放行 | 放行 |
| 改工作目录外的文件（`../secret.txt`、`/etc/hosts`） | 拒绝 | 拒绝 | 放行 |
| 跑命令、其他有副作用的工具（MCP 写操作、写记忆） | 问 | 问 | 放行 |
| 必然造成不可恢复损失的命令（`rm -rf ~`、`mkfs`、`curl … \| sh`、fork bomb 等） | 拒绝 | 拒绝 | 拒绝 |

- 默认模式在 `config.toml` 的 `[permissions] mode` 里改；`sa --mode 只读` 指定这一次；
  REPL 里 `/mode` 看当前模式，`/mode 全放行` 切换，下一次工具调用起生效。
- 工作区模式下跑命令还是要问：没有系统级沙箱，就管不住一条命令会写到哪里。
- 「问」在终端里是一行提问：`y` 允许、`a` 本次会话都允许、其他键拒绝。工作台里是一张审批卡，
  选「允许」「拒绝」或「本次会话始终允许」。
- 拒绝理由会回给模型（包括当前是什么模式、怎么才能放行），它通常能自己换个办法。
...
- `sa run` 里没人确认，默认不许写；定时任务要写记忆就 `--allow memory_write`（或者 `--mode full`）
